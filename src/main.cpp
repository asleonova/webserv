    ...
    while(true)
	{
		...
		for (itSC = arraySC.begin(); itSC != arraySC.end(); itSC++)
			if (FD_ISSET(itSC->ls, &readfds)) {
				...
				// создаем новую сессию (инициируем пустую структуру с дискриптором сессии) -> можно через конструктор структуры
				cur_sess.server_conf = &(*itSC); // ссылка на конфигрурацию сервера, который принял запрос
				cur_sess.sd = sd;
				cur_sess.ss = Session::read;
				// cur_sess.request = "";
				cur_sess.request_obj.clear(); // заполнение сессии (ее очистка)
				cur_sess.answer = "";
				// добавим инфу по адресу и порту клиента в сессию (из возвращенной accept() "sockaddr_in addr")
				cur_sess.client_ip = inet_ntoa(addr.sin_addr);
				cur_sess.client_port = ntohs(addr.sin_port);
				// функция вывода нового соедения
		...
		for (itListSess = listSess.begin(); itListSess != listSess.end(); itListSess++)
		{
			...
			if ((itListSess->ss == Session::read) && (FD_ISSET(itListSess->sd, &readfds)))
			{
				...
				{	
					...
					itListSess->request_obj.syntax(std::string(buf, read_res));

					// DEBUG - выводим статус в консоль
					debug_connect(cur_sess, Session::read_client, read_res);

					// меняем логику - если запрос готов к обработке - то обрабатываем его
					// если нет - продолжаем читать

					// отдаем в структуру "Request" запрос через метод
					// Request cur_request;
					// Request *tmp_request = cur_request.syntax(std::string(buf, read_res).c_str());
					// Request *tmp_request = cur_request.syntax(itListSess->request.c_str());
					// (void)tmp_request;

					// проверяем - полный ли запрос?
					if (itListSess->request_obj._isOK == true)
					{
						...
						// сохраняем строку в буфе ответа и переводим сессию в режим записи
						itListSess->answer_buf = response.getStrForAnswer();

						// если возврат из CGI - берем только body
						// if (true == response.return_from_CGI)
						// 	itListSess->answer_buf = response.body;
						// else
						// 	itListSess->answer_buf = response.getStrForAnswer();
						...
						// предусмотреть - done
						// после отдачи если в запросе было требование закрыть соединение - то закрываем соединение
						// пока соединение может быть закрыто только с клиента
					}
				}
			...
			else if ((itListSess->ss == Session::write) && (FD_ISSET(itListSess->sd, &writefds)))
			{
				...
				if (0 == (itListSess->answer_buf).size())
				{
					...
					if (0 != itListSess->request_obj._headers.count("connection") &&
						"close" == itListSess->request_obj._headers["connection"])
					{
						itListSess->ss = Session::close;
						// DEBUG - выводим статус в консоль
						debug_connect(cur_sess, Session::close_client, 0);
...
std::vector<ServerConf> getSampleServer()
{
	ServerConf				webserv1;	

			webserv1.ip = "127.0.0.1";
			webserv1.port = 8080;
			webserv1.server_name = "my1server";
			webserv1.ls = -1;

			std::cout << "Create server: " << webserv1 << std::endl;

	ServerConf				webserv2;	
	
			webserv2.ip = "127.0.0.1";
			webserv2.port = 9080;
			webserv2.server_name = "my2server";
			webserv2.ls = -1;

			std::cout << "Create server: " << webserv2 << std::endl;

	std::vector<ServerConf> sampleServersSet;

			sampleServersSet.push_back(webserv1);
			sampleServersSet.push_back(webserv2);

	return sampleServersSet;
}
...
int main(int ac, char **av)
{
	...
	// 0. загрузим конфиг
	// std::vector<ServerConf> sampleServersSet = getSampleServer();
	std::vector<ServerConf> sampleServersSet = config.server;

	// 1. инициализируем сетевую часть (создаем сокеты/устанавливаем на порт/слушаем)
	ServerInstanceInit(sampleServersSet);

	// 2. сервера после инициализации
	std::vector<ServerConf>::iterator it;
	for (it = sampleServersSet.begin(); it != sampleServersSet.end(); it++)
		std::cout << "Config server: " << *it << std::endl;

	// 3. стартуем сервера
	ServerGo(sampleServersSet);
